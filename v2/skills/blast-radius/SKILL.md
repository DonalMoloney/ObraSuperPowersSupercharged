...
# blast-radius — measured change risk
...
> an `EVIDENCE:` block (satisfying v1 verification-before-completion's evidence-before-claims rule).

## Triggers

**Use when:**
- "How risky is this change?" — before any review or merge decision
- Preparing to invoke v1 **requesting-code-review** — run this first to calibrate review depth
- Before v1 **finishing-a-development-branch** — verify the blast radius has not grown since the last trace
- Per wave in v1 **dispatching-parallel-agents** — each wave's output should carry a blast-radius score before the next wave starts
...
- A trivially isolated new file has zero inbound edges and its own dedicated test file — skip scoring and proceed directly to v1 **requesting-code-review**

## Algorithm

Build a symbol/import dependency graph from imports and callers (use your language's code-intelligence
tooling; grep fallback below). **Reverse reachability** from changed
...
These are defaults; adjust the thresholds when the codebase has established norms.
...
Build the dependency graph from imports and callers using your language's code-intelligence tooling
(LSP, ctags, or a code-index CLI). If a cached graph is available and the repo has not
...
across all changed symbols — a proxy for interface centrality). If no code-intelligence tooling is
available, fall back to a grep-based callers search (`git grep -rn <symbol>`) and note the fallback in the trace.
...
evidence block (command + literal output). Hand off to v1 **requesting-code-review** with the risk level and reviewer recommendation
pre-populated.
...
EVIDENCE: <dependency trace — tool output, graph node list, or grep results>
```
...
| Emitting a risk level without an `EVIDENCE:` block | The Core rule requires the trace; a bare label with no evidence is not a valid output |

## Proof

Hand off to v1 **requesting-code-review** once the score and recommendation are produced.

The `EVIDENCE:` block must contain:
- The tool or method used to build the dependency graph (e.g., LSP/code-intelligence tooling, `git grep` fallback, `codeindex` CLI)
...
```
EVIDENCE:
  graph tool: codeindex CLI v0.4.1 (reverse-reachability walk)
  ...
  reviewer recommendation: two reviewers, invoke v2 **devils-advocate** before v1 **requesting-code-review**
```
