...
| Status never updated | No review trigger | Set a review date or link the ADR to a v2 **track-assumption** entry for the riskiest consequence |

## Integration

- v2 **track-assumption** — register any unverified assumption embedded in the ADR (e.g. "we assume traffic will not exceed X")
- The ADR directory (`docs/decisions/`) — ADRs are the canonical record; the directory itself is the index
- v1 **writing-plans** — open architectural decisions should be resolved (or registered via v2 **track-assumption**) before the implementation plan is finalised; write the ADR first
- v1 **verification-before-completion** — the ADR itself is evidence of a deliberate decision; cite it in the evidence block when the decision is the claim
