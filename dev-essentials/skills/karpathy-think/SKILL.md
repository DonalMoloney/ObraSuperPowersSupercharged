...
- You are designing an open-ended *feature* that is not yet a concrete change → the **brainstorming** skill owns the heavier design ritual. This skill is the light, per-task check that fires on every coding task once the change is concrete.
- You are choosing *how much autonomy* the work gets — that is a separate decision about leash length. This skill runs *before* the first edit at any autonomy level to make the interpretation explicit: think first, then decide how much latitude the work gets against the interpretation you just wrote.
- You need to name the *verification target* (what "correct" means) before coding — that is a separate gate producing a target artifact, whereas this gate produces a shared understanding. Run this one first — you cannot name a good target for a spec you have silently misread.
...
- This is the precondition for surgical edits and for any rule that downgrades autonomy when "an edit lands outside the predicted file set" — such a rule needs a predicted set to compare against, and this line is it.
