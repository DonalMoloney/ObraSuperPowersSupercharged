...
- You only need to identify the root cause without a full report — use the **systematic-debugging** skill instead.
- The failure was a known flaky test or non-production noise with no user impact — skip the postmortem; file a ticket.
- You are reviewing speculative future risk rather than a real event — that is forward-looking risk mapping, not a postmortem.
...
- The scope is a single code bug with no timeline complexity — use the **systematic-debugging** skill for that.
- The purpose is speculative pre-incident risk mapping rather than reconstructing a real event.
...
Hand off to the **verification-before-completion** skill once the postmortem document is written.
