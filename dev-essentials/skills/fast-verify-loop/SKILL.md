...
**Not this skill if:** you need the loop *run* repeatedly until green — this skill only *chooses* the verifier; drive it yourself afterward. If you don't know the project's canonical build/test/lint commands, detect them from `package.json`/`pyproject.toml`/`Makefile` by hand first. If you need a verification *target* defined (what "correct" even means), settle that before picking how cheaply to check it.
...
- The end-of-task rung from the table is mandatory, not optional polish. When failures need iterating to green, drive the chosen verifier in a tight edit→run loop until it passes.
