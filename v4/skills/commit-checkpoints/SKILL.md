...
tier: v4
inspiration: "Cherny — commit frequently as checkpoints you can roll back to (Claude Code: Best practices for agentic coding, Anthropic engineering blog, April 2025)"
---

# Commit Checkpoints

**Not this skill if:** you are at the end of the work and need to integrate (merge, open the PR, delete the branch) → v1 `finishing-a-development-branch` owns end-of-work; this skill is strictly mid-work. If you need to decide how *big* each increment should be before you verify it, that is K2 short-leash-increments territory (deferred idea, not yet built — keep chunks small ad hoc). This skill assumes the increment is already verified; it governs only the commit that captures it.
...
Checkpoints are private scratch history; the branch that reaches review must read as intentional commits. Before handing off to v1 `finishing-a-development-branch`:
...
- **How this tool operationalizes it:** It turns "commit frequently" into a mid-work discipline with a concrete trigger (what counts as a checkpoint-worthy verified increment), a reversibility-matched cadence ladder, a `checkpoint:` message convention that marks the commits as disposable, a hard never-checkpoint-red rule so every rollback point is safe to land on, and an explicit squash-before-PR handoff to v1 `finishing-a-development-branch`. It pairs with the deferred K2 short-leash-increments (chunk sizing) and is bundled by the `autonomy-control` plugin.
