...
| `v6/` | Runnable AI-powered GitHub Actions (CI/CD) — drop-in workflow templates that call Claude on repo events | Each idea = `v6/<name>/` with a `workflow.yml`, helper `scripts/`, and a `README.md`. **No `SKILL.md`** (not a skill tier — `skill-auditor` does not apply; lint workflows instead). Every workflow MUST declare least-privilege `permissions:`, set `concurrency:`, gate by event/path/label, be idempotent (sticky comments), and route AI calls through `_cost-guardrail` (except where the official `anthropics/claude-code-action` does its own billing). |
| `v7/` | Ports of notable published AI tools/artifacts as runnable Claude Code plugins (starting with Karpathy's) — distinct from v4, which distills *ideas* | Each port = `v7/plugins/<name>/`: an installable plugin (`.claude-plugin/plugin.json` + `commands/` + `skills/` + `scripts/`, optional `hooks/`). Every v7 plugin MUST name the source artifact + release date in a `## Provenance` section and the skill's `inspiration:` frontmatter, preserve the source's core invariant (autoresearch: the harness — not the LLM — owns accept/reject), be runnable out of the box via a deterministic example, and carry `tier: v7` on its skill(s). `skill-auditor` applies to the skill(s); shell harnesses must be `shellcheck`-clean. |
